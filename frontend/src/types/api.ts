...
export type AdminOwnerOption = { id:number; full_name:string; email:string; business_id:number|null }
export type AdminSocialLink = { platform:string; url:string; display_order:number; enabled:boolean }
export type AdminBusinessListItem = { id:number; slug:string; name:string; category:string|null; status:string; owner_user_id:number|null; owner_name:string|null; owner_email:string|null; plan:string; subscription_status:string; review_count:number; complaint_count:number; average_rating:number|null; created_at:string; updated_at:string }
export type AdminBusinessListResponse = { items:AdminBusinessListItem[]; total:number }
export type AdminBusinessDetail = { id:number; slug:string; name:string; logo_url:string|null; description:string|null; category:string|null; google_review_pc_url:string; google_review_mob_url:string; status:string; prefer_ai_comments:boolean; brand_primary_color:string; brand_secondary_color:string; welcome_message:string|null; nfc_enabled:boolean; qr_enabled:boolean; customer_settings:Record<string,unknown>; owner_user_id:number|null; owner_name:string|null; owner_email:string|null; social_links:AdminSocialLink[]; plan:string; subscription_status:string; trial_ends_at:string|null; current_period_start:string|null; current_period_end:string|null; cancel_at_period_end:boolean; review_count:number; complaint_count:number; average_rating:number|null; created_at:string; updated_at:string }
